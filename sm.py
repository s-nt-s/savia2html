...
s = requests.Session()
s.headers = {'User-Agent': "Mozilla/5.0 (X11; U; Linux i686) Gecko/20071127 Firefox/2.0.0.11"}
...
for url in urls:
    ...
    for url2 in urls2:
        ...
        data = {par[0][5:]:par[1] for par in iframe.attrs.items() if par[0].startswith("data-")}
        uuid = data["uuid"]
        piece = piecesindex[uuid]
        courseid = data["courseid"]
        
        dest = "file.php/%s/%s/token/0/%s/%s/%s/%s/%s/" % (piece["modified"], piece["destmodified"], courseid, userid, "ext", data["piecetype"], uuid)
            ...
            for noscript in soup.select("noscript"):
                if noscript.attrs.get("data-type", None) == "content":
                    ...
                    for a in noscript.select("a.sm-media-button"):
                        data = {par[0][5:]:par[1] for par in a.attrs.items() if par[0].startswith("data-")}
                        if "smreference" in data and data["smreference"] in piecesindex:
                            ...
                            if typ == "sm_video":
                                ext = "mp4"
                            btn = "file.php/%s/%s/token/0/%s/%s/%s/%s/%s/" % (piece["modified"], piece["destmodified"], courseid, userid, ext, typ, uuid)
                            btn = urljoin(url2, btn)
                            a.attrs["href"] = btn
                    
                    body.append(noscript)
                    noscript.unwrap()
            css=[style.attrs["href"] for style in soup.select("link") if style.get("href", "").endswith(".css")]
            jss=[script.attrs["src"] for script in soup.select("script") if script.get("src",None)]
        ...
        for e in tpt.select("div.highlight_exercise, div.highlight"):
            divs = [p.parent for p in e.select("span.sm_comp_js_show_more") if p.parent.name=="p"] + e.select("div.sm_js_show_content")
            prs = e.find("h1")
